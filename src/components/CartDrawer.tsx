...
export function CartDrawer() {
  ...
  const { items, isLoading, isSyncing, updateQuantity, removeItem, checkoutUrl, syncCart } = useCartStore();
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <button className="p-1.5 sm:p-2 relative" aria-label="Sacola">
          ...
          <span className="absolute -top-0.5 -right-0.5 bg-primary text-primary-foreground text-[10px] rounded-full size-4 flex items-center justify-center font-bold">
            {totalItems}
          </span>
        </button>
      </SheetTrigger>
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          ...
          <SheetDescription>
            {totalItems === 0 ? "Sua sacola está vazia" : `${totalItems} item(ns)`}
          </SheetDescription>
        </SheetHeader>
            ...
            <>
                ...
                {items.map((item) => (
                  <div key={item.variantId} className="flex gap-4 p-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-semibold">{BRL(parseFloat(item.price.amount))}</p>
                    </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold">{BRL(totalPrice)}</span>
                </div>
                <Button onClick={goCheckout} className="w-full" size="lg" disabled={!checkoutUrl || isLoading || isSyncing}>
